...
- [x] `06-eval-harness.md` — Phase 5 seed: deterministic, precision-first memory eval ✅ (5 task commits `c266d0d`..`9c13224` + 1 hardening fix `676cce4`; generic metric library + `fact-id.ts` mirror, 8-scenario fixtures A–H with deterministic fact-id assertions + flag-gated broken-extraction, `*.eval.ts` suite driving the real mechanism, eval-only vitest config reusing the node:sqlite shim, `pnpm --filter companion eval:memory` + `:check-sensitivity` runners, `docs/history/memory-eval-2026-05.md`; reference run green, broken extraction fails for the right reason (`EXTRACTION_PRECISION_BELOW_THRESHOLD`), eval suite excluded from `pnpm test`; `pnpm verify` green, 453 tests. Codex code-review clean for the eval code — findings were all in earlier MVP code (scoped session finalization, single-user local-only README warning, best-effort unmount). **Follow-up surfaced (non-blocking):** the `sessions` table is keyed on `id` only; re-keying to `(user_id, character_id, id)` guards a `crypto.randomUUID()` collision across scopes that cannot occur in the single-user MVP — deferred as a disproportionate schema migration)

**🎉 Phase 3 memory backlog (01–06) COMPLETE.** Before merging to `main`, run
`git rm -r .hyperclaude/tasks .hyperclaude/research && git commit` (this backlog
and the research files were force-added only to carry context; durable decisions
live in `ROADMAP.md`).
