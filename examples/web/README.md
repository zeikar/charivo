...
The demo also calls `realtimeManager.prepareAudio?.()` from the realtime connect
click before it starts a WebRTC session. Keep that user-gesture path intact on
iOS or the first realtime lipsync pass may stay silent.
