...
const FORMAT_24K: TTSPcmFormat = {
  encoding: "pcm-s16le",
  sampleRate: 24000,
  channels: 1,
};

function createScheduler(format: TTSPcmFormat = FORMAT_24K) {
...
describe("PcmPlaybackScheduler", () => {
  ...
  // The measured trap inherited from the realtime scheduler: the opening chunk
  // of a turn finishes before its successor arrives, so the scheduler drains
  // 3 ms into a twelve-second reply (`tests/gemini-live-smoke/README.md`). The
  // scheduler must report it; the caller is what refuses to call it the end of
  // the utterance.
  it("reports the opening chunk's drain even though the utterance is still arriving", () => {
  ...
  it("queues chunks back-to-back and floors a chunk that lands after the queue ran dry", () => {
    ...
    // One sounding stretch: the transitions are what the caller meters.
  ...
  it("decodes little-endian samples from an unaligned payload", () => {
    ...
    // A chunk sliced out of a byte stream gives no 2-byte alignment guarantee,
    // so the payload can start on an odd byte offset — where an `Int16Array`
    // view throws.
  ...
  // Unlike the realtime scheduler, which is pinned to one vendor rate, this one
  // is handed the format the stream declared.
  it.each([24000, 16000])(
    "builds mono buffers at the format's %i Hz sample rate",
    (sampleRate) => {
...
describe("createPcmPlaybackGraph", () => {
  it("fans the audible output out to the speakers and the lip-sync tap", () => {
    ...
    const graph = createPcmPlaybackGraph(context as unknown as AudioContext);
