...
- ef509fd: Upgrade the `openai` dependency from `^4.47.1` to `^7.10.0`. `openai` is a
...
- 0758343: `LLMClient.call` and `LLMProvider.generateResponse` now take the same
...
- 7bbb53c: Stop history pruning from stranding character replies. `generateResponse(...)`
